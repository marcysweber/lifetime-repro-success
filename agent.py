...
class GeladaAgent(AgentClass):
    ...
    def __init__(self, sex, mother, sire, bandID):
        ...
        super(GeladaAgent, self).__init__(sex, mother, sire)

    def get_rhp(self):
        score = GeladaRhp.rhp[self.rhp][self.age]
        return score


class MakeAgents:
    @staticmethod
    def makenewagent(taxon, sex, mother, sire, population, sim, group, age=0.0):

        if taxon == "savannah":
            newagent = SavannahAgent(sex, mother, sire, group)
        elif taxon == "hamadryas":
            newagent = HamadryasAgent(sex, mother, sire, group)
        elif taxon == "gelada":
            newagent = GeladaAgent(sex, mother, sire, group)
        else:
            raise ValueError("agent without a taxon")
